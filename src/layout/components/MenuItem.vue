  ...
  <div class="menu-item" :class="`menu-item-${depth}`">
    <div
      ...
      :class="
        $route.fullPath === menuItem.path
          ...
          : ''
      "
      @click.stop="handleToggleChildren($event, menuItem)"
    ...
    >
      ...
      <span v-if="menuItem.icon || menuItem.icon_h" class="menu-icon">
        <img
          v-if="$route.path.indexOf(menuItem.path) > -1 || isHover"
          class="icon"
          :src="menuItem.icon_h"
          alt="pic"
        />
      ...
      <i
        ...
        class="mmxlicon mi-arrow-down arrow-status"
        :class="showChildren ? 'open' : 'close'"
      ></i>
    </div>
    <!-- 递归导航列表 -->
    <div
      ...
      @emitChildrenHeight="handleEmitChildrenHeight"
    >
      <menu-item
        v-for="(e, i) in menuItem.children"
        :key="i"
        :menu-item="e"
        :depth="depth + 1"
        @emitChildrenHeight="handleEmitChildrenHeight"
