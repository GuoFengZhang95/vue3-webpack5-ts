<template>
  <div class="dynamic-monitoring-wrp">
    ...
    <div class="scroll-wrp">
      ...
      <!-- 缺省图 -->
      <DefaultArea :loading-status="loadingStatus" :default-img="require('@/assets/images/common/list-default.svg')">
        <template #desc>
          <div class="defaultarea-slot-desc">
            更多动态请到
            <a-dropdown placement="topCenter" :trigger="['click']" overlay-class-name="dashboard-dropdown-menu">
        ...
        this.dynamicList =
          res.data && res.data.list && res.data.list.length
            ? res.data.list.map(item => {
              ...
              if (item.startTime) {
                item.time = `${item.startTime.slice(0, 7).replaceAll('-', '.')}${item.endTime ? '-' + item.endTime.slice(0, 7).replaceAll('-', '.') : '-至今'
                  }`
              }
