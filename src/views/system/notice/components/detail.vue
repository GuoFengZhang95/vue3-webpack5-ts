  ...
  <div class="notice-detail-container">
    ...
    <div class="notice-detail">
      ...
      <div v-if="noticeDetail.ossUploadResultList" class="download-list">
        <div
          v-for="(item, index) in noticeDetail.ossUploadResultList"
          :key="index"
          class="download-item"
          @click="downLoad(item)"
        >
          <img
            v-if="item.type === 'zip' || item.type === 'rar' || item.type === '7z'"
            class="icon"
...
<script>
  import { getNoticeDetail, getAliyunOSSUrl } from '@/api/index.js'
  export default {
    props: {
      externalProp: {
        type: Object,
    ...
    methods: {
      init() {
        ...
        getNoticeDetail({ noticeId: this.externalProp.id })
          .then(res => {
            this.noticeDetail = res.data
            this.$bus.$emit('noticeDetailLoaded', this.externalProp.id)
          })
          .finally(() => {
      ...
      // 下载附件
      downLoad(item) {
        getAliyunOSSUrl({ fileUrl: item.url }).then(res => {
